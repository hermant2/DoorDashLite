...
import com.treyherman.doordashlite.scene.discoverrestaurants.diff.RestaurantDiffCallback
...
class DiscoverRestaurantsPresenter @Inject constructor(
    ...
    companion object {
        private const val DOORDASH_HQ_LAT = 37.422740F
        private const val DOORDASH_HQ_LNG = -122.139956F
    ...
    private fun subscribeToReloadedRestaurants(currentRestaurants: List<UIRestaurant>) {
        restaurantsSingle
            .flatMap { newRestaurants ->
                calculateRestaurantDiffOnce(currentRestaurants, newRestaurants)
            }
            .observeOnMain()
            .subscribeWith(object :
                ...
                override fun onSuccess(restaurantDiffPair: Pair<List<UIRestaurant>, DiffUtil.DiffResult>) {
                    view.hideLoading()
                    view.updateRestaurants(restaurantDiffPair.first, restaurantDiffPair.second)
                }
    ...
    private fun calculateRestaurantDiffOnce(
        ...
        return Single.create<Pair<List<UIRestaurant>, DiffUtil.DiffResult>> {
            val result =
                DiffUtil.calculateDiff(RestaurantDiffCallback(oldRestaurants, newRestaurants), true)
            it.onSuccess(Pair(newRestaurants, result))
        }.subscribeOnComputation()
    }

    private val restaurantsSingle
        get() = restaurantsService.restaurantsByLatLngOnce(DOORDASH_HQ_LAT, DOORDASH_HQ_LNG)
